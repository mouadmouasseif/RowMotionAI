...
export function AppShell({ children, title, subtitle }: { children: React.ReactNode; title: string; subtitle?: string }) {
  ...
  return <main className="dashboard-page">
    ...
    <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}><Brand compact /><nav>{navigationItems.filter((item) => item.roles.includes(profile.role)).map(({ href, label, icon: Icon }) => <Link key={href} onClick={() => setMobileOpen(false)} className={pathname === href || (href !== "/tableau-de-bord" && pathname.startsWith(`${href}/`)) ? "current" : ""} href={href}><Icon />{label}</Link>)}</nav>{pathname !== "/parametres" && <InstallButton compact />}<button className="logout" onClick={async () => { await logout(); router.replace("/"); }}><LogOut />Se déconnecter</button></aside>
    <section className="dashboard-main"><header className="dash-header"><button className="dash-menu" aria-label={mobileOpen ? "Fermer le menu" : "Ouvrir le menu"} onClick={() => setMobileOpen((value) => !value)}>{mobileOpen ? <X /> : <Menu />}</button><div><small>{subtitle ?? "RowMotion AI"}</small><h1>{title}</h1></div><div className="dash-profile"><Link className="icon-link" href="/notifications" aria-label="Notifications"><Bell /></Link><Link className="avatar-link" href="/profil"><ProfileAvatar photoUrl={profile.profilePhotoUrl} firstName={profile.firstName} lastName={profile.lastName} /></Link><div><strong>{name}</strong><small>{profile.role}</small></div></div></header><div className="workspace-content">{children}</div></section>
  </main>;
}
