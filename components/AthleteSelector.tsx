...
export function AthleteSelector({ value, onChange, initialId }: { value: UserProfile | null; onChange: (athlete: UserProfile) => void; initialId?: string | null }) {
  ...
  return <div className="athlete-selector">{profile?.role !== "athlete" && <label className="search-field"><Search /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher un athlète ou une licence" /></label>}{error && <div className="error-card">{error}</div>}<div className="athlete-choice-list">{filtered.map((athlete) => <button type="button" className={value?.uid === athlete.uid ? "selected" : ""} key={athlete.uid} onClick={() => onChange(athlete)}><span className="profile-avatar">{athlete.firstName[0] ?? "A"}{athlete.lastName[0] ?? ""}</span><span><strong>{athlete.firstName} {athlete.lastName}</strong><small>{displayAge(athlete) !== null ? `${displayAge(athlete)} ans` : "Âge non renseigné"} · Licence {athlete.licenseNumber ?? "—"}<br />Club {athlete.clubId ?? "non associé"} · Coach {athlete.coachId ? "associé" : "non associé"}</small></span></button>)}</div>{filtered.length === 0 && !error && <div className="empty-inline"><UserRound />Aucun athlète autorisé.</div>}</div>;
}
