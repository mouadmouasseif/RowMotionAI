...
import { AlertTriangle, ArrowLeft, CheckCircle2, Download, Gauge, MessageSquare, RotateCcw } from "lucide-react";
...
function AnalysisDetail({ analysisId }: { analysisId: string }) {
  const { profile } = useAuth(); const [analysis, setAnalysis] = useState<RowingAnalysis | null>(null); const [error, setError] = useState(""); const [note, setNote] = useState(""); const [localVideoUrl, setLocalVideoUrl] = useState("");
  useEffect(() => { if (!profile) return; void getAnalysis(analysisId, profile).then((value) => { setAnalysis(value); setNote(value.coachComment ?? ""); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Analyse introuvable.")); }, [analysisId, profile]);
  useEffect(() => { if (analysis?.videoStorageMode !== "local") return; let objectUrl = ""; void getLocalAnalysisVideo(analysis.id).then((video) => { if (video) { objectUrl = video.url; setLocalVideoUrl(video.url); } }).catch(() => setLocalVideoUrl("")); return () => { if (objectUrl) URL.revokeObjectURL(objectUrl); }; }, [analysis]);
  if (!profile) return null;
  return <AppShell title="Détail de l’analyse" subtitle={analysis?.athleteName ?? "Chargement"}><Link className="back-link" href="/analyses"><ArrowLeft />Toutes les analyses</Link>{error ? <div className="error-card">{error}</div> : !analysis ? <div className="loading-card">Chargement de l’analyse…</div> : <>
    <section className="analysis-hero-card"><div><span className={`status-pill ${analysis.status}`}>{analysis.status}</span><h2>{analysis.environment === "boat" ? "Analyse en bateau" : "Analyse sur ergomètre"}</h2><p>{analysis.sourceType === "video" ? "Vidéo importée" : "Session en direct"} · {analysis.fileName ?? "Sans fichier"}</p></div><div className="score-ring"><strong>{analysis.technicalScore ?? "—"}</strong><small>/100</small></div></section>
    ...
    <div className="page-actions"><Link className="button ghost" href={`/rapports?analysis=${analysis.id}`}><Download />Préparer le rapport</Link></div>
  </>}</AppShell>;
}
export default function AnalysisDetailPage({ params }: { params: Promise<{ analysisId: string }> }) { const { analysisId } = use(params); return <ProtectedPage><AnalysisDetail analysisId={analysisId} /></ProtectedPage>; }
